...
export const createOCPPHandlers = (
  ...
  GetConfiguration: (id: string, p: any) => {
    ...
    const conf = [
      ...
      { key: 'NumberOfConnectors', readonly: true, value: String(ocppConfig.NumberOfConnectors || 2) },
    ...
    reply(id, {
      configurationKey: keys.length
        ...
        : conf,
      unknownKey: keys.length ? keys.filter((k: string) => !conf.some(c => c.key === k)) : [],
    });
  },
  ...
  ChangeConfiguration: (id: string, p: any) => {
    ...
    const readonlyKeys = ['NumberOfConnectors', 'GetConfigurationMaxKeys', 'SupportedFeatureProfiles', 'WsSecure', 'FirmwareVersion', 'ChargeProfileEnabled', 'ReservationEnabled'];
